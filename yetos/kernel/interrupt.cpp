...
IntHandlerPF(InterruptFrame *frame, uint64_t error_code) {
  uint64_t cr2 = GetCR2();
...
FaultHandlerNoError(DE) FaultHandlerNoError(DB) FaultHandlerNoError(BP)
    ...
    FaultHandlerNoError(MF) FaultHandlerWithError(AC) FaultHandlerNoError(MC)
