...
void TaskWallclock(uint64_t task_id, int64_t data) {
  ...
  auto  clock_window =
      std::make_shared<Window>(8 * 10, 16 * 2, screen_config.pixel_format);
  const auto clock_window_layer_id =
  ...
  auto draw_current_time = [&]() {
    ...
    FillRectangle(*clock_window->Writer(), {0, 0}, clock_window->Size(), {0, 0, 0});
...
extern "C" void KernelMainNewStack(
  ...
  task_manager->NewTask().InitContext(TaskWallclock, 0).Wakeup();
    ...
    case Message::kKeyPush:
      if (auto act = active_layer->GetActive(); act == text_window_layer_id) {
        if (msg->arg.keyboard.press) {
          ...
          msg->arg.keyboard.press && msg->arg.keyboard.keycode == 59 /* F2 */) {
        task_manager->NewTask().InitContext(TaskTerminal, 0).Wakeup();
      } else {
        __asm__("cli");
