...
void Log(LogLevel level, const usb::InterfaceDescriptor &if_desc) {
  Log(level,
      "Interface Descriptor: class=%d, sub=%d, protocol=%d\n",
...
void Log(LogLevel level, const usb::HIDDescriptor &hid_desc) {
  Log(level,
      "HID Descriptor: release=0x%02x, num_desc=%d",
...
usb::ClassDriver *
NewClassDriver(usb::Device *dev, const usb::InterfaceDescriptor &if_desc) {
  ...
  if (dev->DeviceDesc().device_class == 2) {
    ...
    while (auto if_desc = conf_reader.Next<usb::InterfaceDescriptor>()) {
      ...
      for (int i = 0; i < if_desc->num_endpoints; ++i) {
        ...
        if (auto ep_desc =
                usb::DescriptorDynamicCast<usb::EndpointDescriptor>(desc)) {
...
Error Device::StartInitialize() {
  ...
  return GetDescriptor(
      *this,
      kDefaultControlPipeID,
      DeviceDescriptor::kType,
      0,
      buf_.data(),
      buf_.size(),
      true);
...
Error Device::OnControlCompleted(
    ...
    int         len) {
  Log(kDebug,
      "Device::OnControlCompleted: buf 0x%08lx, len %d, dir %d\n",
...
Error Device::InitializePhase2(const uint8_t *buf, int len) {
  ...
  initialize_phase_ = 3;
  Log(kDebug,
      "issuing SetConfiguration: conf_val=%d\n",
