...
void Log(LogLevel level, const TransferEventTRB &trb) {
  if (trb.bits.event_data) {
    Log(level,
        "Transfer (value %08lx) completed: %s, residual length %d, slot %d, ep addr "
        "%d\n",
...
Error Device::ControlIn(
  ...
  Log(kDebug,
      "Device::ControlIn: ep addr %d, buf 0x%08lx, len %d\n",
      ep_id.Address(),
...
Error Device::ControlOut(
  ...
  Log(kDebug,
      "Device::ControlOut: ep addr %d, buf 0x%08lx, len %d\n",
      ep_id.Address(),
      reinterpret_cast<uintptr_t>(buf),
      len);
  if (ep_id.Number() < 0 || 15 < ep_id.Number()) {
...
Error Device::OnTransferEventReceived(const TransferEventTRB &trb) {
  ...
  if (!opt_setup_stage_trb) {
    Log(kDebug,
        "No Corresponding Setup Stage for issuer %s\n",
        kTRBTypeToName[issuer_trb->bits.trb_type]);
    if (auto data_trb = TRBDynamicCast<DataStageTRB>(issuer_trb)) {
      Log(kDebug, *data_trb);
    }
