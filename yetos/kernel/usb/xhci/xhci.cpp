...
Error AddressDevice(Controller &xhc, uint8_t port_id, uint8_t slot_id) {
  ...
  memset(
      &dev->InputContext()->input_control_context,
      0,
  ...
  InitializeEP0Context(
      *ep0_ctx,
      dev->AllocTransferRing(ep0_dci, 32),
...
Error CompleteConfiguration(Controller &xhc, uint8_t port_id, uint8_t slot_id) {
  Log(kDebug,
      "CompleteConfiguration: port_id = %d, slot_id = %d\n",
      port_id,
      slot_id);
...
Error OnEvent(Controller &xhc, CommandCompletionEventTRB &trb) {
  ...
  Log(kDebug,
      "CommandCompletionEvent: slot_id = %d, issuer = %s\n",
      trb.bits.slot_id,
...
void RequestHCOwnership(uintptr_t mmio_base, HCCPARAMS1_Bitmap hccp) {
  ...
  auto ext_usblegsup = std::find_if(extregs.begin(), extregs.end(), [](auto &reg) {
...
void SwitchEhci2Xhci(const pci::Device &xhc_dev) {
  ...
  Log(kDebug,
      "SwitchEhci2Xhci: SS = %02x, xHCI = %02x\n",
      superspeed_ports,
      ehci2xhci_ports);
}
} // namespace
...
Error Controller::Initialize() {
  ...
  if (max_scratchpad_buffers > 0) {
    ...
    Log(kInfo,
        "wrote scratchpad buffer array %p to dev ctx array 0\n",
        scratchpad_buf_arr);
  }
...
Error ConfigureEndpoints(Controller &xhc, Device &dev) {
  ...
  memcpy(
      &dev.InputContext()->slot_context,
      &dev.DeviceContext()->slot_context,
      sizeof(SlotContext));
