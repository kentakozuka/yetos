...
SYSCALL(WinWriteString) {
  return DoWinFunc(
      ...
      arg3,
      arg4,
      reinterpret_cast<const char *>(arg5));
}
...
using SyscallFuncType =
    syscall::Result(uint64_t, uint64_t, uint64_t, uint64_t, uint64_t, uint64_t);
...
void InitializeSyscall() {
  ...
  WriteMSR(
      kIA32_STAR,
      static_cast<uint64_t>(8) << 32 | static_cast<uint64_t>(16 | 3) << 48);
