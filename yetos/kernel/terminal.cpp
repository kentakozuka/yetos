...
fat::DirectoryEntry *
FindCommand(const char *command, unsigned long dir_cluster = 0) {
...
WithError<int> Terminal::ExecuteFile(
  ...
  auto argbuf =
      reinterpret_cast<char *>(args_frame_addr.value + sizeof(char *) * argv_len);
...
void Terminal::Print(const char *s, std::optional<size_t> len) {
  ...
  Message msg =
      MakeLayerMessage(task_.ID(), LayerID(), LayerOperation::DrawArea, draw_area);
  __asm__("cli");
